...
/*
+-----------------------------------------------------------+
| 42 Bit Timestamp | 10 Bit WorkID | 10 Bit Sequence ID |
+-----------------------------------------------------------+
*/
/*
const (
...
/*
+-----------------------------------------------------------+
| 42 Bit Timestamp | 12 Bit WorkID | 10 Bit Sequence ID |
+-----------------------------------------------------------+
*/

const (
	nodeBits  uint8  = 12                    // 节点 ID 的位数
	stepBits  uint8  = 10                    // 序列号的位数
	nodeMax   uint64 = -1 ^ (-1 << nodeBits) // 节点 ID 的最大值，用于检测溢出
	stepMax   uint64 = -1 ^ (-1 << stepBits) // 序列号的最大值，用于检测溢出
	timeShift uint8  = nodeBits + stepBits   // 时间戳向左的偏移量
	nodeShift uint8  = stepBits              // 节点 ID 向左的偏移量
...
//GetTime 获取uid产生的时间
func (n *UUID) GetTime(uid uint64) int64 {
	return int64(uid >> timeShift)
}
